...
- if predictor is too expressive it outwrites the encoder and eventually if the predictor is big enough, there is no meaning for encoder at all
- if sigreg is too big (the weight of sigreg inside the overall loss), then the space is curved
- if sigreg is too small, then the size of the manifold shinks and eventually collapses, but! before the collapse, it can preserve the form
- if sigreg is perfect, then the manifold does not shirk and the spacial information is preserved
- if the predictor problem is linear, simple translation-based predictor does the job
- NICE: if predictor is translation-based, the spacial information is preserved out of the box
- as it is was found in the parer, if the predictor is MLP of some non-linear fuction so the manifold will be curved
